...
exports.default = new Command_1.ButtonInteraction({
    ...
    yield blacklistSchemas_1.default.updateOne({ message: interaction.message.id }, { $set: { status: 'blocked' } });
    return interaction.editReply({
        embeds: [
            new Embed_1.default(client, 'success').setDescription('성공적으로 승인이 완료되었습니다')
        ]
    });
}));
