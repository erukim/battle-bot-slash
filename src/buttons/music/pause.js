...
exports.default = new Command_1.ButtonInteraction({
    ...
    if (guildQueue.connection.paused) {
        ...
        sucessembed.setDescription(`**${(0, builders_1.userMention)(interaction.user.id)}님의 요청으로 일시정지를 해제했어요!**`);
        return interaction.editReply({ embeds: [sucessembed] });
    }
    else {
        guildQueue.setPaused(true);
        sucessembed.setDescription(`**${(0, builders_1.userMention)(interaction.user.id)}님의 요청으로 노래를 일시정지 했어요!**`);
        return interaction.editReply({ embeds: [sucessembed] });
    }
}));
